...
## Содержание:
- [Урок 1. Установка Python и Pycharm](#урок-1-установка-python-и-pycharm)
- [Урок 2. Переменные, оператор присваивания, функции type и id](#урок-2-переменные-оператор-присваивания-функции-type-и-id)
- [Урок 3. Числа и операции над ними](#урок-3-числа-и-операции-над-ними)
- [Урок 4. Математические функции. Модуль math. Функции print() и input()](#урок-4-математические-функции-модуль-math-функции-print-и-input)
- [Урок 5. Логический тип Bool. Операторы сравнения](#урок-5-логический-тип-bool-операторы-сравнения)
- [Урок 6. Введение в строки. Операции над строками](#урок-6-введение-в-строки-операции-над-строками)
- [Урок 7. Индексы и срезы строк](#урок-7-индексы-и-срезы-строк)
- [Урок 8. Основные методы строк](#урок-8-основные-методы-строк)
- [Урок 9. Экранирование символов. Спецсимволы](#урок-9-экранирование-символов-спецсимволы)
- [Урок 10. Форматирование строк и F-строки](#урок-10-форматирование-строк-и-f-строки)
- [Урок 11. Списки. Операторы и функции работы со списками](#урок-11-списки-операторы-и-функции-работы-со-списками)
- [Урок 12. Срезы списков. Сравнение списков](#урок-12-срезы-списков-сравнение-списков)
- [Урок 13. Методы списков](#урок-13-методы-списков)
- [Урок 14. Вложенные списки. Многомерные списки](#урок-14-вложенные-списки-многомерные-списки)
- [Самостоятельная работа 1-14](#самостоятельная-работа-1-14)
- [Урок 15. Условный оператор if. Конструкция if-else](#урок-15-условный-оператор-if-конструкция-if-else)
- [Урок 16. Вложенные условия и множественный выбор. Конструкция if-elif-else](#урок-16-вложенные-условия-и-множественный-выбор-конструкция-if-elif-else)
- [Урок 17. Тернарный условный оператор](#урок-17-тернарный-условный-оператор)
- [Урок 18. Циклы в Python. Цикл While](#урок-18-циклы-в-python-цикл-while)
- [Урок 19. Цикл for](#урок-19-цикл-for)
- [Урок 20. Операторы циклов break, continue](#урок-20-операторы-циклов-break-continue)
- [Урок 21. Примеры задач решаемых с помощью цикла for](#урок-21-примеры-задач-решаемых-с-помощью-цикла-for)
- [Урок 22. Итератор и итерируемые объекты. Функции iter() и next()](#урок-22-итератор-и-итерируемые-объекты-функции-iter-и-next)
- [Урок 23. Вложенные циклы](#урок-23-вложенные-циклы)
- [Урок 24. Генераторы списков. Условия в генераторах списков. Вложенные генераторы списков](#урок-24-генераторы-списков-условия-в-генераторах-списков-вложенные-генераторы-списков)
- [Самостоятельная работа 15-24](#самостоятельная-работа-15-24)
...
## Урок 21. Примеры задач решаемых с помощью цикла for

### Dычисление факториала от натурального числа n.
...
### Отображение символов * в виде елочки.
```
*
**
***
...
### Cоединить все слова списка в одно предложение.
```python
...
### Все двузначные числа в списке заменить нулями.
```python
...
## Урок 22. Итератор и итерируемые объекты. Функции iter() и next()
...
### Функция range() - итерируемый объект
...
5. Поменять строки на столбцы и получить новое представление списка. В математике это называется транспонированием матрицы.
```python
...
## Задачи
1. На вход программе подается натуральное число N (то есть, положительное, целое). Требуется его прочитать и создать двумерный (вложенный) список размером N x N элементов, состоящий из всех единиц, а затем, в последний столбец записать пятерки. Вывести этот список на экран в виде таблицы чисел
2. На вход программе подаются строка: `Desktop   projects opencv   first tuttorial`. Необходимо с помощью вложенных циклов привести ее к виду: `Desktop_projects_opencv_first_tuttorial`.
3. В программе есть двумерный список:
```python
lst = [
[2, 3, 4, 5, 6], 
[3, 2, 7, 8, 9],
...
## Урок 24. Генераторы списков. Условия в генераторах списков. Вложенные генераторы списков
...
```
[выражение for элемент in исходный_список/итерируемый_объект]
```
...
```
### Создание списка с помощью обычного цикла и создание списка с помощью генератора списков.
Создание списка с помощью обычного цикла и создание списка с помощью генератора списков оба позволяют создавать списки, но существуют некоторые различия в синтаксисе и подходе.
...
### Генератор списков с использованием input().split()
...
**Синтаксис**:
```
[выражение for элемент in исходный_список/итерируемый_объект if условие]
```
- **выражение** - это выражение, которое будет применено к каждому элементу исходного списка или итерируемого объекта.
- **элемент** - это переменная, которая будет использоваться для представления каждого элемента исходного списка или итерируемого объекта.
- **исходный_список/итерируемый_объект** - это список или итерируемый объект, на основе которого будет создан новый список.
...
### Несколько циклов for в генератораз спсиков.
...
**Пример 2**:
**Вложенные циклы for в генераторе списка с условием в обоих циклах**
```python

```
В этом примере мы используем два вложенных цикла for в генераторе списка `[x + y for x in list1 for y in list2 if x + y > 5]`, чтобы создать список сумм чисел из двух списков list1 и list2, если сумма больше 5. Первый цикл for x in list1 перебирает значения x из списка list1, а второй цикл for y in list2 перебирает значения y из списка list2. Только те суммы x + y, которые больше 5, добавляются в список sum_list.

**Пример 3**:
**Преобразование двумерного списка в одномерный с помощью вложенных циклов for в генераторе списка**
```python
...
**Пример 4**:
**Генератор списка в качестве итерируемого объекта в генераторе списка**
```python
...
## Задачи:
1. На вход программе подаются вещественные числа, записанные через пробел. Необходимо их прочитать и сохранить в списке lst. Затем, используя list comprehension (генератора списков) сформировать новый список lst_abs из модулей чисел списка lst (в итоговом списке должны храниться именно числа, а не строки). Список lst_abs вывести на экран.
2. На вход программе подается семизначное целое положительное число. Необходимо его прочитать и с помощью list comprehension сформировать список lst, содержащий цифры этого числа (в списке должны быть записаны числа, а не строки). Полученный список вывести на экран.
3. На вход программе подается натуральное число N. Прочитайте его и с помощью list comprehension сформируйте двумерный список размером N x N, состоящий из нулей, а по главной диагонали - единицы. (Главная диагональ - это элементы, идущие по диагонали от верхнего левого угла матрицы до ее нижнего правого угла). Полученный двумерный список вывести на экран.
4. На вход программе подается строка с названиями городов, записанных через пробел. Необходимо прочитать эту строку и сформировать список с помощью list comprehension, содержащий названия городов длиной более пяти символов. Элементы полученного списка вывести в одну строчку через пробел. Пример вводимой строки: `Казань Уфа Москва Челябинск Омск Тур Самара`.
5. На вход программе подается натуральное число n. Необходимо его прочитать и сформировать список с помощью list comprehension, состоящий из делителей числа n (включая и само число n). Элементы полученного списка вывести в одну строчку через пробел. Делителями числа n называются целые числа, которые делят n нацело (без остатка).
6. На вход программе подается натуральное число N. Необходимо его прочитать и сгенерировать вложенный список с помощью list comprehension, размером N x N, где первая строка содержала бы все нули, вторая - все единицы, третья - все двойки и так до N-й строки. Результат вывести на экран.
7. На вход программе подается двумерная таблица из целых чисел
```python
lst = [
  [1, 2, 3, 4],
  [5, 6, 7, 8],
...
```
Необходимо преобразовать его в двумерный (вложенный) список lst, где каждая строка представляется списком из слов (слова разделяются пробелом), но сохранять слова только длиной более трех символов. Решить данную задачу следует с использованием list comprehension. Результат отобразить на экран.
...
2. Какие операторы мы прошли?
3. Что возвращают функции print() и input()?
4. Что такое f-строка?
5. Что такое список в языке программирования Python?
6. Метод добавления в начало списка.
7. Метод удаления последнего элемента списка.
8. Срезы в строках и списках. Синтаксис.
9. Полная конструкция условного оператора if.
10. Что такое тернарный условный оператор?
11. Какие циклы бывают в Python? В чем их основное различие?
12. Что делает оператор break?
13. Что делает оператор continue?
14. Как пройтись по итерируемому объекту с помощью iter() и next()?
15. Что такое вложенные списки?
16. Что такое вложенные циклы?
17. Синтаксис генераторов списков.
...
## Задача 2
### Анкета Пользователя.
В программе есть база данных пользователей, которая представлена следующим образом:
```python
users = [
...
```
Где первый элемент списка - логин пользователя, второй элемент - пароль пользователя, а третий элемент - роль пользователя в программе.  
Написать программу, в которой пользователь может ввести правильный логин (имя) и пароль, чтобы заполнить анкету. Анкета должна состоять из следующих вопросов:
```python
questions = ["Сколько вам лет?", "Чем любите заниматься?", "Ваш любимый фильм?"]
```
После того, как пользователь введет все данные в терминале должна появляться заполненная информация о пользователе:
```
Привет, меня зовут Александр.
Мне 15 лет, я люблю читать книги.
Мой любимый фильм - Операция "Ы".
```
Далее программа заново должна запрашивать логин и пароль другого пользователя.

Пользователь может выйти из программы, введя слово "Выход" в поле логина.

`Усложнение*`: Если пользователь имеет статус "admin", то он может создавать новых пользователей в основном списке вместо заполнения анкеты, заполнив их имя, пароль и роль.

## Урок 25. Словари (dict). Методы словарей. Словари и циклы.
### Словари. Получение значения.
Словари (dictionaries) — это **изменяемые**, **неупорядоченные** коллекции пар **"ключ-значение"**.  
Они используются для **хранения данных**, которые ассоциируются с **уникальными ключами**.  
Словари позволяют быстро получать доступ **к значениям** по их **ключам**.

Словарь определяется с помощью **фигурных скобок** `{}` и содержит пары `ключ-значение`, разделенные двоеточием `:`.  
...
Для получения значения по ключу используется синтаксис `dict[key]`.  
Если ключ **не существует**, будет вызвано исключение `KeyError`.  
Чтобы избежать этого, можно использовать метод `get()`, который возвращает `None` (или другое указанное значение) если ключ не найден.
...
Функция dict() позволяет создавать словари различными способами.
...
### Создание словаря из вложенных списков
Можно создать словарь из вложенных списков или кортежей, где каждый вложенный элемент содержит пару "ключ-значение".
```python
# Вложенные списки
...
Использовать синтаксиса `dict[new_key] = value`, чтобы создать пару "ключ-значение".  
...
print(2 in my_dict)     # Вывод: False (проверяется наличие ключа, а не значения)
```
...
#### Основные методы словаря в Python:
...
5. `items()` - Возвращает представление (view) пар (ключ, значение) словаря.
```python
my_dict = {'a': 1, 'b': 2}
print(my_dict.items())  # dict_items([('a', 1), ('b', 2)])
```

6. `keys()` - Возвращает представление (view) ключей словаря.
```python
my_dict = {'a': 1, 'b': 2}
print(my_dict.keys())  # dict_keys(['a', 'b'])
```

7. `pop()` - Удаляет элемент с указанным ключом и возвращает его значение.
```python
my_dict = {'a': 1, 'b': 2}
value = my_dict.pop('a')
print(value)  # 1
...
8. `popitem()` - Удаляет и возвращает последнюю добавленную пару (ключ, значение).
```python
my_dict = {'a': 1, 'b': 2}
...
9. `setdefault()` - Возвращает значение по ключу, если ключ не существует, добавляет его с заданным значением.
```python
my_dict = {'a': 1}
...
10. `update()` - Обновляет словарь, добавляя пары (ключ, значение) из другого словаря или итерируемого объекта.
```python
my_dict = {'a': 1}
...
11. `values()` - Возвращает представление (view) значений словаря.
```python
my_dict = {'a': 1, 'b': 2}
...
1. **Проход по словарю**  
При проходе по словарю в цикле for по умолчанию перебираются ключи.
```python
my_dict = {'a': 1, 'b': 2, 'c': 3}
...
2. **Проход по ключам словаря**  
Можно явно использовать метод keys() для перебора ключей.
```python
my_dict = {'a': 1, 'b': 2, 'c': 3}
...
for key in my_dict.keys():
    print(key)  # Выводит ключи: a, b, c
```
3. **Проход по значениям словаря**  
Для перебора значений используйте метод values().
```python
my_dict = {'a': 1, 'b': 2, 'c': 3}
...
## Задачи:
1. На вход программе подаются данные в формате ключ=значение, записанные через пробел. Значениями здесь являются целые числа (см. пример ниже). Необходимо прочитать строку с этими данными и на их основе сформировать словарь d, используя функцию dict(). Результирующий словарь вывести на экран. Пример входных данных: `one=1 two=2 three=3`
2. На вход программе подаются данные в формате ключ=значение, записанные через пробел. Необходимо прочитать строку с этими данными и на их основе сформировать словарь. Затем проверить, существуют ли в словаре ключи со значениями: 'house', 'True' и '5' (все ключи - строки). Если все они существуют, то вывести на экран "ДА", иначе "НЕТ". Пример входных данных: `вологда=город house=дом True=1 5=отлично 9=божественно`
3. На вход программе подаются данные в формате ключ=значение, записанные через пробел. Необходимо прочитать строку с этими данными и на их основе сформировать словарь d. Затем удалить из этого словаря ключи 'False' и '3', если они существуют. Ключами и значениями словаря являются строки. Вывести полученный словарь на экран. Пример входных данных: `лена=имя дон=река москва=город False=ложь 3=удовлетворительно True=истина`
4. На вход программе подаются номера телефонов, записанные в одну строчку через пробел, с разными кодами стран: +7, +6, +2, +4 и т.д. Необходимо прочитать строку и на ее основе сформировать словарь d. Ключами словаря должны быть коды (строки: +7, +6, +2 и т. п.), а значениями список номеров в виде строк (следующих в том же порядке, что и в исходной строке) с соответствующими кодами. Полученный словарь вывести на экран. Пример входных данных: `+71234567890 +71234567854 +61234576890 +52134567890 +21235777890 +21234567110 +71232267890`. Пример получившегося словаря: `'+2': ['+21235777890', '+21234567110'], '+5': ['+52134567890'], '+6': ['+61234576890'], '+7': ['+71234567890', '+71234567854', '+71232267890']`
5. На вход программе подается строка из русских букв и символов пробела. Необходимо ее прочитать и закодировать азбукой Морзе, где каждой букве ставится в соответствие код из точки и тире. Полученную строку вывести на экран.
```python
# Словарь морзе
morze = {
...
```
6. На вход программе подается закодированная строка с помощью азбуки Морзе. Коды разделены между собой пробелом. Необходимо ее раскодировать, используя азбуку Морзе из предыдущего занятия. Все буквы в строке должны быть малыми (нижний регистр). Полученное сообщение (строку) вывести на экран. Пример входных данных: `.-- ... . -...- .-- . .-. -. ---`
7. На вход программе подается список целых чисел, записанных в одну строчку через пробел. Необходимо их прочитать и сохранить в виде списка. Затем, с помощью словаря выделите только уникальные (не повторяющиеся) числа. Сформируйте из них еще один список (уникальных чисел). Числа в этом списке должны идти в том же порядке, что и при чтении (из входного потока). Выведите уникальные числа на экран.
8. `*` Сергей собирается в поход и готов взвалить на свои хрупкие плечи максимальный вес в N кг (вводится с клавиатуры). Он решил класть в рюкзак предметы в порядке убывания их веса (сначала самые тяжелые, затем, все более легкие) так, чтобы их суммарный вес не превысил значения N кг. Все предметы даны в единственном экземпляре. Выведите список предметов, которые берет с собой Сергей в порядке убывания их веса. Пример входных данных: `10`. Пример выходных данных: `палатка брезент удочка брюки пила карандаш спички`.
```python
# словарь с наименованиями предметов и их весом (в граммах)
things = {'карандаш': 20, 'зеркальце': 100, 'зонт': 500, 'рубашка': 300, 
          'брюки': 1000, 'бумага': 200, 'молоток': 600, 'пила': 400, 'удочка': 1200,
          'расческа': 40, 'котелок': 820, 'палатка': 5240, 'брезент': 2130, 'спички': 10}
```

## Урок 26. Кортежи (tuple). Методы кортежей.
...
### Методы создания пустого кортежа
- С помощью круглых скобок:
```python
empty_tuple = ()
print(empty_tuple)
...
### Использование синтаксиса срезов в кортежах
...
## Задачи:
1. Объявите в программе следующий кортеж: `t = (3.4, -56.7)`. На вход программе подается последовательность целых чисел, записанных через пробел. Необходимо их прочитать и добавить в конец кортежа t. Добавленные числа в кортеже должны следовать в порядке их считывания. Результат вывести на экран.
2. На вход программе подается строка с названиями городов, записанных через пробел. Необходимо прочитать эту строку и на ее основе сформировать кортеж из названий городов. Названия в кортеже должны идти в том же порядке, что и в исходной строке. Выполните проверку: если в полученном кортеже нет города "Москва", то следует его добавить в конец кортежа. Выведите на экран названия городов из кортежа. Пример входных данных: `Уфа Казань Самара`
3. На вход программе подается строка с названиями городов, записанных через пробел. Необходимо прочитать эту строку и на ее основе сформировать кортеж из названий городов. Затем, выполните проверку: если в полученном кортеже присутствует город "Ульяновск", то этот элемент следует удалить (создав новый кортеж). Выведите на экран названия городов из итогового кортежа. Пример входных данных: `Воронеж Самара Тольятти Ульяновск Пермь`
4. На вход программе подается строка с именами студентов, записанных через пробел. Необходимо прочитать эту строку и на ее основе сформировать кортеж из имен. Затем, отобразите на экране все имена из этого кортежа (по порядку), которые содержат фрагмент "ва" (без учета регистра). Пример входных данных: `Петя Варвара Венера Василиса Василий Федор`
5. На вход программе подаются целые числа, записанные в одну строку через пробел. Необходимо их прочитать и сохранить в кортеже. Затем, создать еще один кортеж с уникальными (не повторяющимися) значениями из первого кортежа. Уникальные числа должны следовать в том же порядке, что и в исходном кортеже. Отобразите найденные уникальные числа. Пример входных данных: `8 11 -5 -2 8 11 -5`
6. На вход программе подаются целые числа, записанные в одну строку через пробел. Необходимо их прочитать и сохранить в кортеже. Затем, в кортеже найти и вывести в одну строчку через пробел (по порядку) все индексы неуникальных (повторяющихся) значений.


## Урок 27. Множества (set). Методы множества.
### Множество в Python?
...
### Какие типы данных можно записывать в множества?
...
### Прохождение по всем элементам множества с помощью цикла for
```python
my_set = {1, 2, 3, 4, 5}
...
## Урок 28. Генераторы множеств. Генераторы словарей
...
### Выделение уникальных значений из списка:
```python
...
## Задача:
В программе определены три словаря, представляющие базу данных пользователей:
```python
logins = {
    1: "user1",
    2: "user12",
...
**Индексы** в словарях соответствуют **одному пользователю**, где:
- **logins** — логины пользователей,
- **passwords** — пароли пользователей,
- **roles** — роли пользователей ("user" или "admin").
### Напишите программу, которая будет:
1. Запрашивать у пользователя логин и пароль.
2. Если роль пользователя — "user", программа должна запрашивать и заполнять следующие данные:
- **Полное имя**,
- **Номер телефона**,
- **Профессия**.
3. Если роль пользователя — "admin", программа должна позволять добавлять нового пользователя, запрашивая:
- **Логин**,
- **Пароль**,
- **Роль**.
### Условия работы программы:
- Программа должна продолжать работу, пока пользователь не введет `exit` или `Exit` в качестве логина.
- В конце работы программы необходимо **вывести всех пользователей с заполненными данными (только с ролью "user")**.
- Если какой-либо **пользователь заполнил данные несколько раз**, выводить нужно **только последние данные**.

## Урок 29. Функции. Оператор return. Аргументы и параметры функции.
### Что такое функции?
**Функции** в Python — это блоки кода, которые **выполняют определенную задачу** и могут быть **вызваны в любом месте программы**.  
...
- **Повторного использования кода**: Один раз написанный код можно использовать многократно.
- **Упрощения отладк**и: Легче найти и исправить ошибки в небольших блоках кода.
- **Улучшения читаемости**: Код становится более организованным и понятным.
- **Модульности**: Функции позволяют разбивать программу на логические части.

### Внутри функции можно использовать:
- Переменные
- Операторы (условные, циклы и т.д.)
- Другие функции
- Исключения
- Встроенные и пользовательские объекты

### В Python нельзя использовать функцию до ее объявления. Функция должна быть определена до того, как она будет вызвана.
### Определение функции:
```python
def greet(name):
    print(f"Hello, {name}!")
...
```
### Примеры функций:
1. Функция с **созданием переменных** внутри функции:
```python
def create_variables():
    a = 10
    b = 20
    c = a + b
    print(f"Сумма a и b: {c}")

create_variables()
```
2. Функция с **использованием условного оператора**:
```python
...
check_number(0)
```
3. Функция с **использованием другой функции внутри функции**:
```python
def inner_function():
...
outer_function()
```
4. Функция с **использованием исключения внутри функции**:
```python
...
divide_numbers(10, 2)  # Результат деления: 5.0
divide_numbers(10, 0)  # Ошибка: Деление на ноль!
divide_numbers(10, 'a')  # Ошибка: Неверный тип данных!
```
...
Порядок передачи параметров имеет значение, так как **позиционные аргументы должны соответствовать параметрам по порядку**.
#### Пример:
```python
def display_info(name, age):
    print(f"Name: {name}, Age: {age}")

display_info("Alice", 30)  # Правильно
display_info(30, "Alice")  # Неправильно, порядок перепутан
```
...
print(result)  # 8
```
#### в функции может отработать только один оператор return, после чего выполнение функции прекращается.

### Функции с несколькими return в условном операторе.
```python
def check_number(num):
    if num > 0:
        return "Positive"
...
### Из функции можно вернуть несколько значений, используя кортеж.
```python
def get_coordinates():
    x = 10
    y = 20
...
### Использование функций при возврате результата из другой функций.
```python
def add(a, b):
    return a + b
...
### Именные и позиционные аргументы функции.
- **Позиционные аргументы** передаются в функцию **по порядку**.
- **Именные аргументы** передаются **с указанием имени параметра**.
Пример:
```python
def display_info(name, age=30):
    print(f"Name: {name}, Age: {age}")

display_info("Alice")  # Name: Alice, Age: 30
display_info("Bob", 25)  # Name: Bob, Age: 25
display_info(age=40, name="Charlie")  # Name: Charlie, Age: 40
```
...
display_info("Alice", city="New York")  # Правильно
# display_info(city="New York", "Alice")  # Ошибка
```

### Пустой список [ ] в аргументах.
Если передать пустой список как значение по умолчанию, он будет общим для всех вызовов функции, что может привести к неожиданным результатам.
```python
...
print(append_to_list(2))  # [1, 2] — неожиданный результат
...
2. Как вызвать функцию?
3. В каком месте кода можно вызывать функцию?
4. Сколько раз можно вызывать функцию в программе?
5. Что можно использовать внутри функции?
6. Что такое параметры функции?
7. Что такое аргументы функции?
8. Что делает оператор return внутри функции?
9. Сколько операторов return могут отработать внутри одной функции?
10. Можно ли вернуть несколько значений из одной функции?
11. Что такое позиционные аргументы функции?
12. Что такое именные аргументы функции?
...
5. Объявите в программе функцию с именем get_sq, которая имеет один параметр (принимает вещественное число). В теле функции значение параметра возводится в квадрат и возвращается функцией. После объявления функции прочитайте (с помощью функции input) вещественное число из входного потока и вызовите функцию с прочитанным значением. Выведите на экран число, которое возвратила функция.
6. Объявите функцию с именем is_triangle, которая принимает три стороны треугольника (целые числа) и проверяет, можно ли из переданных аргументов составить треугольник. (Напомню, что у любого треугольника длина любой его стороны должна быть меньше суммы двух других). Если  проверка проходит, функция должна возвращать булево значение True, а иначе False.
7. Объявите в программе функцию с одним параметром для проверки переданного числа на четность. Функция должна возвращать булево значение True, если переданное число четное и False, в противном случае. После объявления функции в цикле на каждой итерации необходимо читать целочисленное числовое значение. Цикл должен работать пока x не равен 1. Если прочитанное значение x четное (проверяется с помощью ранее объявленной функции), то число выводится на экран (каждое с новой строки).
8. Объявите в программе функцию с одним параметром для проверки переданного числа на нечетность. Функция должна возвращать булево значение True, если переданное число нечетное, и False в противном случае. После объявления функции далее в программе прочитайте последовательность целых чисел, подаваемых на вход программе. Затем, используя генератор списков и объявленную ранее функцию, сформируйте список lst из нечетных значений на основе списка lst_d. Результат отобразите на экране.
9. Объявите в программе функцию, которая имеет один параметр, принимающий строку. Функция должна возвращать булево значение False, если длина переданной строки меньше 6 символов, иначе возвращать булево значение True. После объявления функции далее в программе прочитайте из входного потока строку с названиями городов, записанных через пробел. Сформируйте на основе прочитанной строки список cities из названий городов. Затем, используя генератор списка и ранее объявленную функцию, сформируйте новый список lst из названий городов с длинами не менее шести символов (города выбираются из списка cities). Результат отобразите на экране.
10. Объявите функцию, которая имеет один параметр, принимающий строку. Функция должна возвращать два значения в виде кортежа: переданную строку и ее длину. После объявления функции далее в программе прочитайте из входного потока строку с названиями городов, записанных через пробел. Сформируйте на основе прочитанной строки список cities из названий городов. Затем, используя генератор словарей и ранее объявленную функцию, сформируйте на основе списка cities словарь d в формате: `d = {<город 1>: <число символов>, ..., <город N>: <число символов>}`. Выведите этот словарь.
11. На вход программе подаются целые числа, записанные в одну строчку через пробел. Необходимо их прочитать и сохранить в списке digs. Затем (или в самом начале программы), объявить функцию, которая имеет два параметра (будут передаваться максимальное и минимальное значения из сформированного списка digs) и возвращающую произведение двух переданных аргументов. Вызовите эту функцию с передачей ей в качестве аргументов минимального и максимального числового значения из списка digs. Отобразите на экране значение, возвращенное функцией.
12. Объявите в программе функцию с именем get_rect_value, которая первыми двумя параметрами принимает длину и ширину прямоугольника (числа), а третий формальный параметр tp имеет начальное значение 0. Если параметр tp равен нулю, то функция должна возвращать периметр прямоугольника, вычисленного на основе первых двух переданных аргументов, а иначе его площадь.
13. Объявите в программе функцию с именем check_password, которая первым параметром принимает строку (пароль) и имеет второй формальный параметр chars с начальным значением в виде строки "$%!?@#". Функция должна проверять, есть ли в пароле хотя бы один символ из chars и что длина пароля не менее 8 символов. Если проверка проходит, то функция возвращает булево True, иначе False.
14. Объявите в программе функцию, которая первым параметром принимает строку (с кириллицей и латиницей) и преобразовывает в ней кириллические символы в латиницу, используя следующий словарь для замены русских букв на соответствующее латинское написание: `t = {'ё': 'yo', 'а': 'a', 'б': 'b', 'в': 'v', 'г': 'g', 'д': 'd', 'е': 'e', 'ж': 'zh',
     'з': 'z', 'и': 'i', 'й': 'y', 'к': 'k', 'л': 'l', 'м': 'm', 'н': 'n', 'о': 'o', 'п': 'p',
     'р': 'r', 'с': 's', 'т': 't', 'у': 'u', 'ф': 'f', 'х': 'h', 'ц': 'c', 'ч': 'ch', 'ш': 'sh',
     'щ': 'shch', 'ъ': '', 'ы': 'y', 'ь': '', 'э': 'e', 'ю': 'yu', 'я': 'ya'}`. Функция должна возвращать результат преобразования переданной строки в латиницу. Замены делать без учета регистра (исходную строку вначале следует перевести в нижний регистр - малые буквы). Второй формальный параметр функции с именем sep и начальным значением в виде строки "-". Он определяет символ для замены пробелов в строке. На вход программе подается строка, которую необходимо прочитать (после объявления функции). Затем, дважды вызовите функцию (с выводом результата ее работы на экран): первый раз только с прочитанной строкой, второй раз с прочитанной строкой и именованным аргументом sep со значением '+'.
15. Объявите в программе функцию, которая первым параметром принимает строку, а второй формальный параметр tag  с начальным значением в виде строки "h1" определяет тег, в который должна заключаться первая переданная строка. Например, мы передаем строку "Hello Python" и заключаем в тег "h1". Функция должна вернуть строку (без кавычек): `<h1>Hello Python</h1>`. После объявления функции далее в программе прочитайте из входного потока строку и дважды вызовите функцию (с выводом результата ее работы на экран): первый раз только со строкой, второй раз со строкой и именованным аргументом tag со значением 'div'.